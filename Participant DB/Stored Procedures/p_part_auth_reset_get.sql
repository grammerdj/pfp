...
-- ---------------------------- --
# Script for Creating User Credential Reset Get Proc
-- ---------------------------- --
...
DROP PROCEDURE IF EXISTS `p_part_auth_reset_get`;
DELIMITER //
CREATE
    PROCEDURE `p_part_auth_reset_get` (IN `p_email_otp` VARCHAR(100),
								IN `p_pin` INT,
                                IN `p_user` VARCHAR(100),
                                OUT `p_stat_c` VARCHAR(1))
	COMMENT "Participant password reset GET procedure"
get_sq: BEGIN
	/* Standard Proc Logging Variables */
	DECLARE beg_ts DATETIME DEFAULT NOW();
	DECLARE proc_name VARCHAR(100) DEFAULT "p_part_auth_reset_get";
	DECLARE params TEXT DEFAULT CONCAT("[",COALESCE(p_email_otp, "NULL"),",",COALESCE(p_pin, "NULL"),",",COALESCE(p_user, "NULL"),"]");
    ...
    SELECT @email:=a.email_otp, a.previous_pwds, a.sq_1, a.sq_2 FROM t_part_auth as a
		INNER JOIN t_part_ref as r ON a.ref_id = r.ref_id 
        WHERE a.email_otp = p_email_otp AND r.pin = p_pin;
    IF @email IS NULL
    THEN
		SET p_stat_c = "F";
	ELSE
